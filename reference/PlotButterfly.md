...
with three modes:
  ...
  - `"bar"`: stacked horizontal bar chart (e.g. Risk vs Protective
...
## Arguments
  ...
  - type:
    ...
    Chart type: `"bar"` (default, stacked), `"bar_dodge"`
    (side-by-side), or `"violin"`.
  ...
  - palette:
    ...
    In bar mode, colours map to `fill.by` levels. In violin mode,
    colours map to each variable.
  ...
  - alpha.by:
    ...
    only). When provided, the alpha of each bar is scaled by this
    column's values (e.g. cell type percentage), producing darker bars
    for higher values. Overrides the fixed `alpha`. Default `NULL`
    (uniform alpha).
  ...
  - minmax:
    ...
    only). Useful when comparing methods that produce values on
  ...
  - legend\_theme:
    ...
    `theme_legend1()`. When provided, legend formatting is applied via
    `fmt_legend`. Default `NULL`.
...
Other plot: `PlotButterfly2()`, `PlotRankCor()`, `plt_cat()`,
