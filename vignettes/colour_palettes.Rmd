...
All palettes are stored in `palette_list` (accessible via `UtilsR::palette_list` or `pal_get()`):
...
`n` fixes how many colours each palette contributes, following the same rules
as `pal_get()`. `max_colors` still caps how many of them are drawn:

```{r pal-show-n, eval=FALSE}
